...
const fetchUserInfo = async (setUser: any) => {
    try {
        ...
        if (response.data.success) {
            const userResponse = await axios.get(`${envConfig.NEXT_PUBLIC_API_ENDPOINT}/users`, { withCredentials: true });
                ...
                <CarouselContent>
                    {PRODUCT.map((item: any, index: any) => (
                        <CarouselItem key={index} className="basis-1/4">
                            <Link href={`product/${item.id}`} className="block bg-white text-black rounded-xl relative overflow-hidden hover:text-red-500 cursor-pointer">
                                <div className=" relative h-[300px] overflow-hidden ">
                                    <Image src={item.url} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" alt="" className=" hover:scale-125 duration-500" />
                                </div>
                                <div className="p-3">
                                    ...
                                    <div className="absolute top-[10px] left-0 bg-red-700 w-[50px] h-7 rounded-r-lg">
                                        <div className="flex items-center justify-center w-full h-full text-white">{item.discount}%</div>
                                    </div>
                                </div>
                            </Link>
                        </CarouselItem>
